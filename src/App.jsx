...
You’ve got the culprit sitting there in plain sight, like a raccoon on the kitchen counter.

At the very bottom of your WATCHaware component, this block is what renders the floating “🔄 Test State” button on every screen:
...
Your return should end like this instead:

      </div>
    </div>
  );
}
If you want to keep it for development but hide it in production, use this instead:
...
One other thing. You mentioned that overlapping white box issue earlier. In this file, the spacing most likely comes from this section inside AppScreen:

<div style={{ display: "flex", flexDirection: "column", alignItems: "center", flex: 1, justifyContent: "center", gap: 28, width: "100%", padding: "20px 0" }}>
and this in HoldButton:

marginTop: 65, marginBottom: 10
If the white box under the green circle is crowding upward, change them to:

gap: 36
and:

marginTop: 85, marginBottom: 20
That will push the lower content down without doing some cursed -18px padding hack. Negative padding isn’t valid CSS anyway, because apparently standards still matter in this one tiny corner of civilization.

Clean fix summary:

1. Delete the DEV TEST BUTTON block entirely.
2. Increase AppScreen gap from 28 to 36.
3. Increase HoldButton marginTop from 65 to 85.
4. Increase HoldButton marginBottom from 10 to 20.
Here are the exact two edits:
...
That should remove the floating test button and give the lower white panel more breathing room.


i need to remove the test state button all together, i need to move the background up 18px and i need to the .metrics-card down 90px

Do these three edits. Human beings do love hiding the problem at the bottom of the file.

First, remove the Test State button entirely.

Delete this whole block from the bottom of WATCHaware():
...
So the end of your component should now look like:

      </div>
    </div>
  );
}
Second, move the background image up 18px.
...
Third, move the metrics card down 90px.

You don’t actually have a .metrics-card class in this file. The metrics card is this div inside the caretaker section of AppScreen:
...
please rewrite the code and look for bugs
...
i want the complete app.jsx file with changes

Here’s the full App.jsx with the requested changes baked in:

removed the Test State button completely

moved the background image up 18px

moved the metrics card down 90px

fixed a couple of small logic annoyances while we were in there:

0 values now display correctly instead of being turned into null

the “Skip for now” location button no longer pretends it granted location access

added a tiny safety cleanup for the SOS hold interval
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ fontSize: 13, color: COLORS.slateLight, lineHeight: 1.7, marginBottom: 28 }}>
          When you press the SOS button, your location will be shared with your caregivers so they
    ...
    >
      ...
      >
        ...
        <circle
          ...
          style={{
            cursor: "grab",
            filter: "drop-shadow(0 3px 8px rgba(245,200,66,0.6))",
          }}
          onMouseDown={(e) => {
            e.preventDefault();
        ...
        <circle
          ...
          style={{
            cursor: "grab",
            filter: "drop-shadow(0 3px 8px rgba(137,164,199,0.6))",
          }}
          onMouseDown={(e) => {
            e.preventDefault();
...
function AppScreen({
  ...
  useEffect(() => {
    ...
    const granted = localStorage.getItem("watchaware_location_granted");
    if (!granted) setTimeout(() => setShowLocPrompt(true), 1200);
    else setLocationReady(granted === "true");
  }, [isPatient]);
  ...
  return (
    <AppBackground tier={alertTier} sosFired={sosFired}>
      ...
      >
        ...
        >
          <Logo subtitle={`${isPatient ? "Patient" : "Caregiver"}  ·  ${time}`} light={light} onClick={onCycleTier} />
          {isCaretaker && (
            <button
              onClick={onSettings}
              style={{
            ...
            >
              {!sosFired && !cancelled && (
                ...
                >
                  ...
                  {"\n"}
                  {locationReady
                    ? "Your caregivers will be notified with your location."
